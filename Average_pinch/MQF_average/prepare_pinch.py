...
def kern(i, mydict, lock, state_list):
    print(f'Running: {i+1}')
    with open('stdout'+str(i)+'.out','w') as f:
        with contextlib.redirect_stdout(f):
            return one_pinch(mydict, lock, N_pinches=n_pinches_to_average, save_sigmas_and_coords=False, idd=i+1, grid=None, state_list=state_list)
...
i0 = one_pinch(result_dict, lock, N_pinches=n_pinches_to_average, save_sigmas_and_coords=True, grid=grid_dict, state_list=state_list)

with multiprocessing.Pool(workers) as pool:
    result_list = pool.starmap_async(kern, [(ii, result_dict, lock, state_list) for ii in range(n_pinches_to_average-1)])
...
print(f"max x : {grid_dict['xg'][-1]/grid_dict['sigma_x_beam']}sigmas")
print(f"max y : {grid_dict['yg'][-1]/grid_dict['sigma_y_beam']}sigmas")
print(f"max z : {grid_dict['zg'][-1]/grid_dict['sigma_z_beam']}sigmas")
print(f"Sigma_x: {grid_dict['sigma_x_beam']}")
print(f"Sigma_y: {grid_dict['sigma_y_beam']}")
print(f"Sigma_z: {grid_dict['sigma_z_beam']}")
