...
coll_deg = 43.772
disable_sextupoles = False

if disable_sextupoles:
...
plt.plot(x_line, - np.tan(coll_deg*np.pi/180.)*x_line + coll_r/np.cos((coll_deg)*np.pi/180.), color='k', linewidth=2.0)
plt.plot(x_line, - np.tan(coll_deg*np.pi/180.)*x_line - coll_r/np.cos((coll_deg)*np.pi/180.), color='k', linewidth=2.0)
#plt.plot(x_line, np.tan((90.-coll_deg)*np.pi/180.)*x_line - coll_r/np.cos((180-coll_deg)*np.pi/180.))
plt.xlim(-0.0,10)
plt.ylim(-0.0,10)
plt.xlabel('$\\mathbf{A_1\ [\\sigma]}$')
plt.ylabel('$\\mathbf{A_2\ [\\sigma]}$')
plt.title('$\\mathbf{\\epsilon_n = 1.7 \\mu m,\ non-linear\ LHC,\ off-momentum}$')
plt.show()
