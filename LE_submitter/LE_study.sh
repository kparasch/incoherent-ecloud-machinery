...
#
#job_name=LE_LHC_450GeV${MB_name}${MQ_name}_${intensity}e11ppb_${qx}qx_${qy}qy_${qprime}qprime_${IMO}IMO_${VRF}VRF_${ptau_max}ptau
#./submit_le_ecloud_v1.0.sh --job_name=${job_name} --ptau_max=${ptau_max} \
#                     --qx=${qx} --qy=${qy} --qprime=${qprime} --IMO=${IMO} --VRF=${VRF} \
#                     --seyMB=${seyMB} --seyMQ=${seyMQ} ${MB} ${MQ} --intensity=${intensity}

#for intensity in 1.10 0.60
for intensity in 1.20 
do

    for ptau_max in 0.00e-4 3.92e-4 5.48e-4 6.63e-4 7.55e-4 8.31e-4
    do
        MB=--MB ; [[ "$MB" == "--MB" ]] && MB_name=_${seyMB}seyMB || MB_name=;
        MQ=--MQ ; [[ "$MQ" == "--MQ" ]] && MQ_name=_${seyMQ}seyMQ || MQ_name=;
        job_name=LE_LHC_450GeV${MB_name}${MQ_name}_${intensity}e11ppb_${qx}qx_${qy}qy_${qprime}qprime_${IMO}IMO_${VRF}VRF_${ptau_max}ptau
