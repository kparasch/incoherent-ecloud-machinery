...
copy_destination=$repository/Tracking_Data/LE_450GeV
tracking_arguments="--jobtype 'LE' --copy_destination ${copy_destination} --simulation_input ${line} \
--ptau_max ${ptau_max} --max_tau ${max_tau} --output ${job_name}.h5 --n_sigma 5.7 \
--skip_turns ${skip_turns} --turns_per_checkpoint ${turns_per_checkpoint} --last_checkpoint ${last_checkpoint} \
...
if [ "$restart_job" == true ]
then
    cp -r ${job_name} ${job_name}_restarted
    job_name=${job_name}_restarted
fi
