...
parser = argparse.ArgumentParser(description='FMA simulations with e-clouds')
...
ecloud_lattice = ec_stl.LatticeWithEclouds(line, eclouds_info, ps, ecloud_types=ecloud_types, device=device)
ecloud_lattice.set_optics_CO(optics, partCO)

ecloud_lattice.add_tricub_data(pinch_path, 'mqd', max_z=max_tau)
ecloud_lattice.add_tricub_data(pinch_path, 'mqf', max_z=max_tau)
ecloud_lattice.remove_dipolar_kicks()

tricub_to_tricub_data = {}
for key in eclouds_info['length'].keys():
    this_type = eclouds_info['type'][key]
    if this_type in ecloud_types:
        tricub_to_tricub_data[key] = this_type
...
kfm.dict_to_h5(output_to_save, output_file)
