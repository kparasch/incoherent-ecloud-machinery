...
n_particles = 10000
n_sigma = 10
ptau_max = 1.e-4*float(sys.argv[2])
epsn_1 = 1.7e-6
epsn_2 = 1.7e-6
seed = 1
...
for i,elname in enumerate(line.element_names):
    if elname[0:10] == 'ecloud.mbb' and elname.split('.')[2] == arc:
#        print(elname)
        line.elements[i] = pysixtrack.elements.BeamMonitor(num_stores=n_turns, is_rolling=True)
        monitors += 1
...
ymax = 0
tmax = 0
job = sixtracklib.TrackJob( elements, ps, device='opencl:0.3')
first_turns = 1
...
#print(f'Max( x: {1000*xmax:.6f} mm, y: {1000*ymax:.6f} mm, zeta: {1000*tmax:.6f}) mm, {np.sum(mask)} particles survived')
print(f'{10000*ptau_max:.6f}, {1000*xmax:.6f}, {1000*ymax:.6f}, {1000*tmax:.6f}')
        #print(f'Max( x: {1000*xmax:.6f} mm, y: {1000*ymax:.6f} mm, zeta: {1000*tmax:.6f}) mm, {np.sum(mask)} particles survived, Turn: {i}, Mon: {j}')
